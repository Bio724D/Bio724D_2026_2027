...
three_genes <- yeast_long |> 
  filter(gene %in% c("YER002W", "YLR197W", "YFL039C")) |>
  filter(expt == "cdc28") |> 
  mutate(name = case_when(gene == "YER002W" ~ "NOP16",
                          gene == "YLR197W" ~ "NOP56",
                          gene == "YFL039C" ~ "ACT1"))
```

```{r}
ggplot(data = three_genes, aes(x = time, y = name, fill = expression)) +
    geom_tile() +
    scale_fill_gradientn(limits=c(-1, 1), colors=color.scheme) +
    theme_bw()
```
...
```{r}
interactive_plot <- ggplot(data = tooltip_data, aes(x = time, y = name, fill = expression, text = tooltip_text)) +
    geom_tile() +
    scale_fill_gradientn(limits=c(-1, 1), colors=color.scheme) +
    theme_bw()
...
```{r}
subplot(tree_plot, interactive_plot, widths = c(0.3, 0.6))
...
microbescope |> 
  ...
  fmt_percent(columns = case_fatality_rate, scale_values = FALSE, drop_trailing_zeros = TRUE) |> 
  fmt_number(columns = average_basic_reproductive_rate, drop_trailing_zeros = TRUE) |> 
  cols_label(
