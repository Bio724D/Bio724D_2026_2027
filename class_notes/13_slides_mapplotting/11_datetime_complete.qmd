...
Working with dates and times presents several challenges. First, date and time values are recorded in many different formats. Second, math with date and time values presents challenges due to inconsistent and non-decimal numbering systems. Third, time zones and daylight savings time complicate coordinating values and doing math with values. (See the slide deck for a fuller explanation of these challenges.)

The tidyverse package `lubridate` is designed to make these problems easier to manage. Base R does provide functions to work with date and time objects, but they have limited utility compared with `lubridate` functions. In addition, `lubridate` functions are designed to work seamlessly with the rest of the `tidyverse`.

Refer to chapter 17 in *R for Data Science* (2nd ed) and the slide deck for this week's class for background information and additional examples. Refer to the `lubridate` cheat-sheet for quick reference.

## Set-up

The only package we need to install is `tidyverse`, since `lubridate` is a standard component. Some older videos and books may instruct you to load `lubridate` separately, as it was initially a separate package, but this is no longer necessary. However, be aware that if you are working with legacy code that requires an older version of R, you may still need to load `lubridate` separately.
...
typeof(my_datetime)
```
To coerce strings into date and date-time objects, use the constructor functions `as_date` and `as_datetime`. 
```{r}
...
```
Date objects are designated as class "Date", which is easy to remember. Date-time objects are designated as class "POSIXct", which is less obvious; the term derives from the POSIX standard for implementations of UNIX, while "ct" is short for "calendar time".
...
Rabbit hole: You can also retrieve the current date and time using the base R function `date`. Note that the base R function `time` does *not* return the current time, but instead generates a vector of time samples from a data frame. You can use the base R function `Sys.timezone` to retrieve the current time zone. For reasons of simplicity and consistency, it's usually best to use the `lubridate` functions.   

**Specifying a different time zone.** By default, `today` and `now` use the local time zone. Optionally, you can provide a different time zone. The following provides the same information for Universal Coordinated Time (UTC), also known as Greenwich Mean Time (GMT), Universal Time, and Zulu Time. Either 3-letter abbreviation works. 
```{r}
# retrieve the current date
today(tzone="UTC")
# retrieve the current date and time
...
- Continent/city or ocean/island (e.g., "Asia/Tokyo" and "Pacific/Tahiti"). Many options!    
...
- Multiple synonyms for GMT: "GMT", "GMT0", "GMT+0", "GMT-0", "UTC", "Universal", "Zulu"
...
If you don't know the time zone descriptor, an alternative approach is to use the offset from GMT. The code below retrieves the current date-time for 2 hours later than GMC 
```{r}
now(tzone="Etc/GMT-2")
```
...
A common data cleaning task is converting dates and date-times to **ISO 8601** format, the international standard. The process of conversion can be challenging because dates and times are represented in so many different formats. 

`lubridate` provides a family of helper functions that simplify the conversion. For example, the function `dmy` converts a string where the first value is the date, the second value is the month, and the third value is the year and returns a date object in ISO 8601-format: year-month-date.
...
**Creating date objects.** This family of functions has a different version for every possible order of units, including `myd`, which you are very unlikely to need! 
```{r}
...
**Extracting components of a date or date-time object.** The code below takes as input the following date-time object generated above: "2022-03-22 03:30:09". Note that the day of the month is retrieved by `mday` (or its alias `day`), because `date` is another function.
```{r}
...
```{r}
extract_7 <- dst(test_3)                  # is it during daylight savings? (logical)
extract_7
extract_8 <- am(test_3)                   # is it during the morning? (logical)
extract_8
...
For the `month` and `wday` extraction functions, it is also possible to return values as strings. Use the optional argument `label=TRUE` to turn on names, and the optional argument `abbr=FALSE` to display full names.
```{r}
...
floored
ceilinged <- ceiling_date(my_datetime, unit="days") 
ceilinged
```
...
Calculating time spans is complicated. Units differ in their "base" (60, 24, 7, ~30, ~365, etc.). Daylight savings time as well as leap years and leap seconds add further complexity. 
...
A **duration** is a time span that is defined only by its length. It is simply a number of seconds. Durations ignore dates, time zones, and irregularities like leap years and daylight savings time. A simple way to think about a duration is that it is the time that you would see if you used a stopwatch to measure time. The stopwatch doesn't care what time of day it is, what day of the year it is, or what time zone it is in. All that matters is how many seconds have elapsed.
...
For small time units, the conversion into seconds is exact. For example, 1 minute = 60 seconds. For the two largest time units, the conversion is approximate: `lubridate` uses the "average" of 365.25 days to represent 1 year and 4.35 weeks to represent 1 month.
```{r}
dur_4 <- dmonths(2)
dur_4
```
...
**Creating period objects.** To create a period object, use a function with the plural of the unit name. For instance, use `minutes` to create a period object with units in minutes.
```{r}
per_1 <- hours(4)
per_1
class(per_1)
...
per_6
```
**Math with durations *vs* periods.** Notice the different result when adding a duration compared with adding a period to the current date. Adding a period produces a more human-friendly result, namely the same date, two months from now. Adding duration might give a different date (depending on whether the two month have 28, 29, 30, or 31 days), and it always converts the date object to a date-time object in order to accommodate the extra seconds that are used to represent an "average" month.   
```{r}
...
```{r}
intv_6 <- ymd("2025-08-25") %--% ymd("2025-11-25")   # fall semester for graduate courses
today() %within% intv_6                              # is today in the fall semester?
ymd("2025-12-12") %within% intv_6                    # what about the last day of the year? 
```
...
```{r}
intv_7 <- ymd("2025-11-27") %--% ymd("2024-11-30")  # Thanksgiving break
int_overlaps(intv_6, intv_7)                        # TG break overlap with the semester?
intv_8 <- ymd_h("2025-08-25 00") %--% ymd_h("2025-08-26 00")  # FDOC                       
int_aligns(intv_6, intv_8)                          # FDOC align with the semester?
```
These logical returns can be very useful for filtering rows or as condition in control flow structures.