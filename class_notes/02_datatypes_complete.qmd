...
var_1 <- 3.14159           # assignment is "silent" (no feedback)
var_1                      # typing a variable name displays its value
```

To learn the data type of an object, use the `typeof()` function:

```{r}
typeof(var_1)               
```

By default, R will always create a float if you assign a number, even if there is no decimal point. 
...
Name <- c("Anika", "Bob", "Chen")
Age <- c(24, 25, 23)
Student <- c(TRUE, FALSE, TRUE)

# now, create the data frame
df <- data.frame(Name, Age, Student)     # notice the '.'
df
```
...
typeof(df)
```
A data frame is built using the list data structure, which why R considers it to be a list. However, a data frame has additional properties and rules (e.g., every column be the same length), so R considers it to be a special kind of list. More about this later.
...
```{r}
#View(penguins)     # note: only works in RStudio!
```

More useful is the function `head()`, which displays just the first six rows. This is particularly useful with data frames that are too long to fit on the screen.
...
```
R will not give you a warning or throw an error in any of these cases. It's good to check data type and values after assignment to make sure they are what you want, particularly while you're becoming familiar with R.
...
In general, if there are *any* `NA` values in a column, a calculation on that column will return `NA`. This can quickly become annoying. (Note: `tidyverse` functions are an important exception; most ignore `NA` values and give you a warning.)
...
```
The following command creates a logical vector with `TRUE` at every position where a column contains an `NA` value, and `FALSE` everywhere else. This can be very useful for filtering data, as we will see in the following section. 
...
Vectors and lists can be filtered using "Boolean indexing". This involves two steps. First, construct a vector of truth values. Typically, this is done using a condition that you specify. Second, apply this Boolean index to either a vector or a column from a data frame. Where the indexing vector contains a value of `TRUE`, the corresponding value in the object you want to filter is returned. Importantly, the lengths of the Boolean index and the vector or column being filtered must match.

This is easiest to see using a simple vector:

```{r}
# here is a vector I want to filter
x <- c(1, 2, 3, 4, 5, 6)

# first, construct the boolean index (here by hand, but normally by condition)
my_bool_vect <- c(TRUE, FALSE, FALSE, TRUE, TRUE, FALSE)

# second, apply the boolean index to the filter using square bracket indexing
x[my_bool_vect]
```

Only the values of `x` that match up with `TRUE` values in the Boolean index are returned.

For a more realistic example, consider finding missing values, as we did in the previous section: `is.na(penguins$body_mass_g)`
This line of code returns a vector of logical values, based on a condition. In this case, the condition is whether there is missing data. 

However, any condition can be used to create a vector of logical values. For instance, we can create a Boolean index for large penguins:

```{r}
my_bool_vec <- penguins$body_mass_g > 3600
my_bool_vec
```

We can now use this vector of logical values to filter for rows where penguins weigh more than 3,600 g by passing it to the row value of square bracket indexing.

```{r}
penguins[my_bool_vec, ]
```

It works, but this is a two-step process that could be done more efficiently in one step. So why would we want to use Boolean indexing? The short answer is that filtering with a Boolean vector is computationally very efficient. If you want to use a filter repeatedly, you only need to construct the Boolean vector once, saving time and keeping code simpler. We will see examples later on.   
